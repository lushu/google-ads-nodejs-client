...
describe('v15.SharedSetServiceClient', () => {
    describe('Common methods', () => {
        it('has apiEndpoint', () => {
            const client = new sharedsetserviceModule.v15.SharedSetServiceClient();
        ...
        if (typeof process !== 'undefined' && typeof process.emitWarning === 'function') {
            it('throws DeprecationWarning if static servicePath is used', () => {
                ...
                const servicePath = sharedsetserviceModule.v15.SharedSetServiceClient.servicePath;
            ...
            it('throws DeprecationWarning if static apiEndpoint is used', () => {
                const stub = sinon.stub(process, 'emitWarning');
                const apiEndpoint = sharedsetserviceModule.v15.SharedSetServiceClient.apiEndpoint;
                assert.strictEqual(apiEndpoint, 'googleads.googleapis.com');
                assert(stub.called);
                stub.restore();
            });
        ...
        it('sets apiEndpoint according to universe domain camelCase', () => {
            const client = new sharedsetserviceModule.v15.SharedSetServiceClient({universeDomain: 'example.com'});
            const servicePath = client.apiEndpoint;
            assert.strictEqual(servicePath, 'googleads.example.com');
        });

        it('sets apiEndpoint according to universe domain snakeCase', () => {
            const client = new sharedsetserviceModule.v15.SharedSetServiceClient({universe_domain: 'example.com'});
            const servicePath = client.apiEndpoint;
            assert.strictEqual(servicePath, 'googleads.example.com');
        });
        it('does not allow setting both universeDomain and universe_domain', () => {
            assert.throws(() => { new sharedsetserviceModule.v15.SharedSetServiceClient({universe_domain: 'example.com', universeDomain: 'example.net'}); });
        });

        it('has port', () => {
            const port = sharedsetserviceModule.v15.SharedSetServiceClient.port;
            assert(port);
    ...
    describe('mutateSharedSets', () => {
        it('invokes mutateSharedSets without error', async () => {
            ...
            const request = generateSampleMessage(
              new protos.google.ads.googleads.v15.services.MutateSharedSetsRequest()
            );
            const defaultValue1 =
              getTypeDefaultValue('.google.ads.googleads.v15.services.MutateSharedSetsRequest', ['customerId']);
            request.customerId = defaultValue1;
            const expectedHeaderRequestParams = `customer_id=${defaultValue1}`;
            const expectedResponse = generateSampleMessage(
              new protos.google.ads.googleads.v15.services.MutateSharedSetsResponse()
            );
        ...
        it('invokes mutateSharedSets without error using callback', async () => {
            ...
            const promise = new Promise((resolve, reject) => {
                 client.mutateSharedSets(
                    request,
                    (err?: Error|null, result?: protos.google.ads.googleads.v15.services.IMutateSharedSetsResponse|null) => {
                        if (err) {
                            reject(err);
                        } else {
                            resolve(result);
